...
| 📱 | **移动优先** | 重点适配手机和平板竖屏；PWA 会在每台设备本地缓存书架、应用外壳和访问过的封面。 |
...
├── books/            # EPUB 原文件
└── covers/
    ├── thumbnails/   # 384px / 768px 版本化 WebP 封面
    └── ...            # 升级前已有的原始封面
```
...
首次启动新版容器时，应用会自动升级 `library.sqlite`，然后以低优先级逐本生成
384px 和 768px 的 WebP 封面。处理过程具有以下特性：

- 不搬动或重命名 `books/` 中的 EPUB。
- 一次只处理一本书；书架可同时正常使用。
- 中途重启容器后会跳过已完成项目并继续，不会从头重做。
- 旧封面会保留，缩略图尚未完成时仍可作为回退。

日志出现 `Cover thumbnail backfill finished` 表示本轮处理结束。升级后建议先正常使用一段时间并完成备份。若确认缩略图齐全、也不需要回滚到旧镜像，可先预览能够释放的空间：
...
> [!WARNING]
> 清理旧封面不可由应用自动撤销，而且会让不认识新缩略图字段的旧镜像失去封面。
> 请只在备份完成并确认不再回滚后执行；不执行清理不影响新版使用。

## 📱 添加到手机主屏幕

EPUB Reader 提供 PWA 启动外壳和每设备独立缓存：

- **iPhone / iPad：** 使用 Safari 打开应用，点击“分享” → “添加到主屏幕”。
- **Android：** 使用 Chrome 打开应用，在菜单中选择“安装应用”或“添加到主屏幕”。

第一次打开时，设备会保存应用外壳和书架快照；封面在实际显示时按需写入该设备的缓存。
以后启动会先显示本地快照，再在后台向服务器确认是否有更新。封面缓存采用最近最少使用
淘汰策略，上限为 500 项（设计目标约 100 MiB，实际空间由封面大小和浏览器配额决定）。
手机、平板和每台电脑互不共享浏览器缓存，因此新设备第一次访问仍需要下载一次。

断网时可以浏览最近成功加载的书架、文件夹和已缓存封面；EPUB 正文没有加入离线缓存，
因此不要把它当作完整的离线阅读功能。浏览器或系统也可能在空间紧张、清除网站数据时
回收缓存，服务器中的书库始终是唯一持久数据源。

> [!IMPORTANT]
> Service Worker 只在安全上下文中工作：正式从手机或其他电脑访问时请通过 HTTPS 反向代理
> 打开应用。`localhost` 可在开发时使用 HTTP；直接访问
> `http://192.168.x.x:4080` 仍可在线使用应用，但不会获得完整的 PWA 本地缓存能力。
